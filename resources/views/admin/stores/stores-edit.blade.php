    ...
    <div class="admin-form-modern">
        <div class="admin-form-card">
            ...
            <div class="admin-form-body">
                <form method="POST" action="{{ route('admin.stores.stores.update', $store->id) }}" id="storeEditForm" onsubmit="return validateAndSubmitForm(event)">
                        @csrf
                        @method('PUT')
                    ...
                    <div class="form-section">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $store->name) }}" placeholder="Masukkan nama toko" required>
                                    @error('name')
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control @error('short_name') is-invalid @enderror" id="short_name" name="short_name" value="{{ old('short_name', $store->short_name) }}" placeholder="Contoh: Samsae Jogja" maxlength="20" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control @error('code') is-invalid @enderror" id="code" name="code" value="{{ old('code', $store->code) }}" placeholder="Contoh: ABC001" required>
                                    @error('code')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control @error('owner_name') is-invalid @enderror" id="owner_name" name="owner_name" value="{{ old('owner_name', $store->owner_name) }}" placeholder="Masukkan nama pemilik" required>
                                    @error('owner_name')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email', $store->email) }}" placeholder="email@example.com" required>
                                    @error('email')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="tel" class="form-control @error('phone') is-invalid @enderror" id="phone" name="phone" value="{{ old('phone', $store->phone) }}" placeholder="08123456789" required>
                                    @error('phone')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <textarea class="form-control @error('address') is-invalid @enderror" id="address" name="address" rows="3" placeholder="Jl. Contoh No. 123, RT/RW 001/002" required>{{ old('address', $store->address) }}</textarea>
                                    @error('address')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <input type="text" class="form-control @error('province') is-invalid @enderror" id="province" name="province" value="{{ old('province', $store->province) }}" required>
                                @error('province')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                        </div>
                                ...
                                <input type="text" class="form-control @error('city') is-invalid @enderror" id="city" name="city" value="{{ old('city', $store->city) }}" required>
                                @error('city')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                        </div>
                                ...
                                <input type="text" class="form-control @error('postal_code') is-invalid @enderror" id="postal_code" name="postal_code" value="{{ old('postal_code', $store->postal_code) }}" required>
                                @error('postal_code')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                        </div>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  const oldProv = {{ old('loc_provinsi_id', $store->loc_provinsi_id) ?: 'null' }};
  const oldKab = {{ old('loc_kabkota_id', $store->loc_kabkota_id) ?: 'null' }};
  const oldKec = {{ old('loc_kecamatan_id', $store->loc_kecamatan_id) ?: 'null' }};
  const oldDesa = {{ old('loc_desa_id', $store->loc_desa_id) ?: 'null' }};
  
  // Debug: Log old values
  console.log('Old location values:', { oldProv, oldKab, oldKec, oldDesa });
  ...
  function loadKab(pid, restoring){
    ...
    fetch(`{{ url('/api/locations/kabkotas') }}/${pid}`).then(r=>r.json()).then(j=>{
      ...
      (j.data||[]).forEach(it=>{ $kab.append(new Option(it.name, String(it.id), false, false)); });
      ...
      setTimeout(function() {
        if(restoring && oldKab !== null && oldKab !== undefined && oldKab !== 'null'){ 
          console.log('Setting Kab/Kota value:', oldKab, 'Type:', typeof oldKab);
          const kabValue = String(oldKab);
          // Check if option exists
          const optionExists = $kab.find('option[value="' + kabValue + '"]').length > 0;
          console.log('Kab/Kota option exists:', optionExists, 'Available options:', $kab.find('option').length);
          
          if (optionExists) {
            $kab.val(kabValue).trigger('change'); 
            console.log('Kab/Kota value after set:', $kab.val());
            // Wait a bit more before loading next level
            setTimeout(function() {
              loadKec(kabValue,true); 
            }, 150);
          } else {
            console.warn('Kab/Kota option not found for value:', kabValue);
          }
        } else {
          console.log('Skipping Kab/Kota restore:', { restoring, oldKab, type: typeof oldKab });
        }
      }, 150);
    });
  }
  function loadKec(kid, restoring){
    ...
    fetch(`{{ url('/api/locations/kecamatans') }}/${kid}`).then(r=>r.json()).then(j=>{
      ...
      (j.data||[]).forEach(it=>{ $kec.append(new Option(it.name, String(it.id), false, false)); });
      if (window.jQuery && $.fn.select2) { 
  ...
  function loadDesa(did, restoring){
    ...
    fetch(`{{ url('/api/locations/desas') }}/${did}`).then(r=>r.json()).then(j=>{
      ...
      (j.data||[]).forEach(it=>{ $desa.append(new Option(it.name, String(it.id), false, false)); });
      if (window.jQuery && $.fn.select2) { 
  ...
  // Validate and submit form - ensure coordinates are saved
  window.validateAndSubmitForm = function(event) {
    // Ensure latitude and longitude are set from map marker
    ...
    const locProv = document.getElementById('loc_provinsi_id').value;
    const locKab = document.getElementById('loc_kabkota_id').value;
    const locKec = document.getElementById('loc_kecamatan_id').value;
    const locDesa = document.getElementById('loc_desa_id').value;
    
    if (!locProv || !locKab || !locKec || !locDesa) {
    ...
    console.log('Submitting form with:', {
      loc_provinsi_id: locProv,
      loc_kabkota_id: locKab,
      loc_kecamatan_id: locKec,
      loc_desa_id: locDesa,
  ...
  function initMap() {
    const storeLat = {{ $store->latitude ?? 'null' }};
    const storeLng = {{ $store->longitude ?? 'null' }};
    const initialLat = parseFloat(document.getElementById('latitude').value) || storeLat || defaultLat;
    const initialLng = parseFloat(document.getElementById('longitude').value) || storeLng || defaultLng;
