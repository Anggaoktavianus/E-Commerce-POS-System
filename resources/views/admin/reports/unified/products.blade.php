...
@section('title', 'Laporan Penjualan Produk Terpadu')
...
<div class="container-xxl flex-grow-1 container-p-y">
  ...
  <div class="card page-header-card mb-4">
    <div class="card-body">
      <div class="d-flex justify-content-between align-items-center flex-wrap">
        <div>
          <h4 class="mb-1">
            <i class="bx bx-package me-2 text-primary"></i>Laporan Penjualan Produk Terpadu
          </h4>
          <p class="text-muted mb-0">Gabungan penjualan produk dari online dan POS</p>
        </div>
  ...
  <div class="card mb-4">
    <div class="card-body">
      <form method="GET" action="{{ route('admin.reports.unified.products') }}" class="row g-3">
        <div class="col-md-4">
  ...
  <div class="card">
    ...
    <div class="card-body">
        ...
        <table class="table table-hover table-bordered">
          ...
          <tfoot>
            <tr class="table-primary">
              ...
              <th class="text-end">{{ number_format($productSales->sum('online_quantity'), 0, ',', '.') }}</th>
              <th class="text-end">Rp {{ number_format($productSales->sum('online_total'), 0, ',', '.') }}</th>
              <th class="text-end">{{ number_format($productSales->sum('pos_quantity'), 0, ',', '.') }}</th>
              <th class="text-end">Rp {{ number_format($productSales->sum('pos_total'), 0, ',', '.') }}</th>
              <th class="text-end">{{ number_format($productSales->sum('total_quantity'), 0, ',', '.') }}</th>
              <th class="text-end">Rp {{ number_format($productSales->sum('total_sales'), 0, ',', '.') }}</th>
            </tr>
          </tfoot>
        </table>
      </div>
      ...
      <div class="alert alert-info">
        <i class="bx bx-info-circle me-2"></i>Tidak ada data penjualan produk pada periode yang dipilih
      </div>
      @endif
    </div>
  </div>
