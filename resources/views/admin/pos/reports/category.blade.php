...
@section('title', 'Laporan Penjualan Kategori - POS')
...
<div class="container-xxl flex-grow-1 container-p-y">
  ...
  <div class="card page-header-card mb-4">
    <div class="card-body">
      <div class="d-flex justify-content-between align-items-center flex-wrap">
        <div>
          <h4 class="mb-1">
            <i class="bx bx-category me-2 text-primary"></i>Laporan Penjualan Kategori
          </h4>
          <p class="text-muted mb-0">Penjualan per kategori</p>
        </div>
        <div class="d-flex gap-2">
          <a href="{{ route('admin.pos.dashboard') }}" class="btn btn-secondary">
            <i class="bx bx-arrow-back me-1"></i>Kembali
          </a>
          <a href="{{ route('admin.pos.reports.export', ['type' => 'category', 'outlet_id' => $outletId, 'date_from' => $dateFrom, 'date_to' => $dateTo]) }}" class="btn btn-success">
  ...
  <div class="card mb-4">
    <div class="card-body">
      <form method="GET" action="{{ route('admin.pos.reports.category') }}" class="row g-3">
        <div class="col-md-3">
    ...
    <div class="col-md-4">
      <div class="card bg-primary text-white">
        <div class="card-body">
          ...
          <h3>{{ number_format($categorySales->sum('total_quantity'), 0, ',', '.') }}</h3>
        </div>
      </div>
    </div>
    <div class="col-md-4">
      <div class="card bg-success text-white">
        <div class="card-body">
          ...
          <h3>Rp {{ number_format($categorySales->sum('total_sales'), 0, ',', '.') }}</h3>
        </div>
      </div>
    </div>
    <div class="col-md-4">
      <div class="card bg-info text-white">
        <div class="card-body">
          ...
          <h3>{{ number_format($categorySales->count(), 0, ',', '.') }}</h3>
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="card-body">
      ...
      <div class="table-responsive">
        <table class="table table-hover">
          ...
          <tbody>
            @php $totalSales = $categorySales->sum('total_sales'); @endphp
            ...
            <tr>
              ...
              <td class="text-end">{{ $totalSales > 0 ? number_format(($item->total_sales / $totalSales) * 100, 2) : 0 }}%</td>
            </tr>
            @endforeach
          </tbody>
        </table>
